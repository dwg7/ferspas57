...
**What that turned into, concretely** (the paragraph above is the founding intent; this is the built reality — see `README.md` "The technical question"): not a static STAC→catalog generator, but a working end-to-end vertical slice. Real FERSPAS-published GAEZ v5 and Hand-in-Hand rasters are converted once into 22 PMTiles sources (served via Martin at `stars.optgeo.org`) plus 13 GeoJSON site files, rendered by a MapLibre Cartographer at <https://dwg7.unopengis.org/ferspas57/>, with a 373-entry narrative library and a Staff system prompt on top. The redirect happened for two measured reasons: FERSPAS has no orthophoto collections at all, and browser access to its COG assets is inconsistent between datasets (GAEZ v5's bucket sends `Access-Control-Allow-Origin: *`, Hand-in-Hand's does not — measured 2026-09-11, D63), so a `/catalog` pointing at them would have rendered only the half a browser can reach.
...
- **Respect FERSPAS's own terms exactly.** The STAC catalog documented in `HANDOVER.md` mixes CC-BY, CC-BY-SA, CC-BY-NC-SA, and various "proprietary-but-often-effectively-open" FAO licenses across its 1282 collections (the "~157" figure from hfu's June 2026 manual survey is confirmed stale — see `HANDOVER.md`). Never assume open-by-default; check and record the specific license of whatever collection this repo actually ingests (see `HANDOVER.md`'s open item on this).
- **Build something FAO's own team could look at and recognize as useful**, not just useful to DWG 7. A working tile interface that correctly represents real FERSPAS-published data, with correct attribution and license metadata, is itself a small contribution back to the FERSPAS ecosystem — anyone building a lightweight STAC consumer could reuse the same conversion patterns. (This originally read "FERSPAS's orthophoto collection." That premise is dead: a full paged survey of all 1282 collections found **no orthophoto or aerial-imagery collection anywhere in FERSPAS**, and the one externally-provided German DOP dataset present in June 2026 has since been removed — see `HANDOVER.md`. What got built instead is GAEZ v5 + Hand-in-Hand, converted end to end. Don't go looking for orthophotos.)
