...
2. **A `/catalog` endpoint alone would not have been usable across the layers we needed.** FERSPAS's COG assets live in per-dataset Google Cloud Storage buckets, and browser access to them is inconsistent: as measured on 2026-09-11, GAEZ v5's bucket serves `Access-Control-Allow-Origin: *` and can be range-read directly from a browser, while Hand-in-Hand's does not send the header at all. A map that needs both — which is exactly what this project's narratives do, GAEZ classification underneath an HIH score — cannot be built on direct COG access while half its layers are unreachable to a browser. A TileJSON document pointing at them would have looked correct and rendered only half.
